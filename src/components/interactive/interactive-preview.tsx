 ...
 * customer taps them on their own device). Its only copy is the
 * empty-state fallbacks, which live in the shared `Interactive`
 * namespace so it can be dropped into the composer, the automation
    ...
    >
      ...
      {payload.kind === "buttons" ? (
          ...
          {payload.buttons.map((b, i) => (
            ...
            >
              ...
              <span className="truncate">
                {b.title || t("previewButtonFallback")}
              </span>
            </button>
          ))}
        </div>
        ...
        >
          ...
          <span className="truncate">
            {payload.button_label || t("previewMenuFallback")}
          </span>
        </button>
      )}
    </div>
