...
export async function POST(request: Request) {
  try {
    ...
    // Tazeyo HR CRM is single-tenant-per-WhatsApp-number — letting two accounts
    ...
    if (claimed) {
      return NextResponse.json(
        {
          error:
            'Bu WhatsApp telefon numarası bu kurulumdaki başka bir hesaba zaten bağlı. Her telefon numarası yalnızca tek bir Tazeyo HR CRM kullanıcısına bağlanabilir.',
        },
        { status: 409 }
      )
    }
